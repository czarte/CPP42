...
def format_result(label: str, result: int | float, okcolor: str):
    ...
    return (
        ...
        else f"{label}{okcolor}{result_str}{C.ENDC}".ljust(ALIGN)
        + f"{C.OKGREEN}OK{C.ENDC}"
    )
...
def valid_range(input_range: str):
    ...
    ranges = [
        extract_range(range_str) for range_str in re.split(r"\s*,\s*", input_range)
    ]
    if not all(ranges):
        raise argparse.ArgumentTypeError(
            f"Not a valid range: {C.WARNING}{input_range}{C.ENDC}. "
...
def run_test(
    ...
    test_input: list[str],
    output_check: bool,
    maximal_number_of_comparisons: int,
):
    ...
    number_of_comparisons_match = re.search(
        r"^Number of comparisons: [0-9]+$", cleaned_ouput, re.MULTILINE
    )
    ...
    if output_check:
        valid_output_match = re.search(
            r"^\s*After:\s*\[?((?:\d+\s*)+)\]?\s*$", cleaned_ouput, re.MULTILINE
        )
        ...
        if len(split_output) != len(test_input):
            return (
                ErrorCode.OUTPUT_NUMBERS_DIFFER_FROM_INPUT,
                " ".join(split_output),
                " ".join(test_input),
            )
        ...
        if split_output_int != expected:
            return (
                ErrorCode.OUTPUT_NUMBERS_NOT_SORTED,
                " ".join(split_output),
...
if __name__ == "__main__":
    ...
    for i, test_range in enumerate(ranges_to_test):
        ...
        maximal_number_of_comparisons = calculate_number_of_maximal_comparisons(
            len(test_range)
        )
        ...
        worst_result_str = format_result(
            "Worst result:                ", worst_result, C.OKGREEN
        )

        best_result = min(results)
        best_result_str = format_result(
            "Best result:                 ", best_result, C.OKBLUE
        )

        average_result = sum(results) / len(results)
        average_result_str = format_result(
            "Average result:              ", average_result, C.OKCYAN
        )
